...
package Generic_Router is
   ...
   task type Router_Task (Task_Id  : Router_Range := Draw_Id) is
      ...
      entry Receive_Routertables (PRB : in Router_tables; sender : in Router_Range); ---- send routertables to other routers

      entry Receive_unit         (Me  : in Message_unit); ----- send message  to other routers
